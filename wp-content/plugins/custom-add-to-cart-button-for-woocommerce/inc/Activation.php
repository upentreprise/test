...
class Activation
{
    ...
    {
    	//try to get from wp option first
	    $option_data = self::get_license_details_from_user_option();
        ...
        {
            $data = json_decode($wp_filesystem->get_contents(self::get_license_file()), true);
            update_option(Config::LICENSE_DATA_OPTION, wp_json_encode($data));
    ...
    private static function get_license_details_from_user_option()
    {
    	$raw_data = get_option(Config::LICENSE_DATA_OPTION, '');

    	if ($raw_data === '')
    		return false;

	    return json_decode($raw_data, ARRAY_A);
