...
class Talemy_Block_Posts extends Talemy_Block {
        ...
        if ( 'author' == $this->atts['tab_type'] ) {
            ...
            if ( 'subcat' == $this->atts['tab_type'] ) {
                ...
                if ( !empty( $this->atts['categories'] ) ) {
                    $args['child_of'] = is_array( $this->atts['categories'] ) ? $this->atts['categories'][0] : '';
                }
            }
	...
	public function get_content() {
        ...
        while( $this->query->have_posts() ) {
            $this->query->the_post();
            get_template_part( 'templates/loop/' . $this->atts['list_style'] );
        }
    ...
    function get_preload_content() {
        if ( 'preload' == $this->atts['preload_content'] && !empty( $this->tab_ids ) ) {
            ...
            foreach ( $this->tab_ids as $tab_id ) {
                ...
                $query = new WP_Query( $query_args );//error_log( json_encode( $query->found_posts ) );
